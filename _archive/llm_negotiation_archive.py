...
#------------------------------------------------------------after first suggestions from copilot 
from server.config import *
import json
import pandas as pd
import logging
...
def propose_activity_change(params):
    ...
    # Analyze voting
    _, _, _, _, voting, _, _ = load_csvs()
    bbq_voters = voting[(voting["space"] == space_id) & (voting["activity"] == current)]
    sports_voters = voting[(voting["space"] == space_id) & (voting["activity"] == desired)]
    ...
    explanation = f"Residents who voted for {current} in {space_id}: {list(bbq_residents)}. Residents who voted for {desired}: {list(sports_residents)}. Overlap: {list(overlap)}."
    # Check geometry suitability (dummy logic)
    geometries, *_ = load_csvs()
    space = geometries[geometries["id"] == space_id]
    if not space.empty:
        area = space.iloc[0]["area"]
        explanation += f" Current area: {area}. (Dummy: check if suitable for {desired}.)"
    # Decision
    if overlap:
...
def find_profile_swap(params):
    ...
    # Find spaces with desired activity
    geometries, _, _, _, voting, distances, personas = load_csvs()
    assignments = None
    try:
    ...
    if assignments is not None:
        sports_spaces = assignments[assignments['assigned_activity'] == desired]['space_id'].tolist()
    # Find nearby residents to those spaces
    swap_candidates = []
    for space_id in sports_spaces:
        if space_id in distances['id'].values:
            # Find closest resident (dummy: just pick one)
            for col in distances.columns:
                if col != 'id':
                    swap_candidates.append({"space_id": space_id, "resident": col})
    explanation = f"Spaces with {desired}: {sports_spaces}. Swap candidates: {swap_candidates}. (Dummy logic)"
    if swap_candidates:
        explanation += "\nDo you want to proceed with a swap?"
        return {"result": explanation, "can_proceed": True, "params": params}
    else:
...
def negotiation_flow(user_query, user_id=None):
    """
    Main negotiation flow: analyzes user query, data, and suggests actions. User can pick, loop, or finalize.
    """
    # 1. Explain situation to user (context)
    context = {}
    if user_id:
    ...
    # 2. Use LLM or rules to suggest possible actions (from 4 main actions)
    # For now, use static/dummy suggestions as before
    suggestions = [
        {
            "action": "propose_activity_change",
            "explanation": "You can negotiate with residents who voted for Barbeque in O10. If some of them also like Sports, you may be able to swap the activity.",
            "parameters": {"space_id": "O10", "current_activity": "Barbeque", "desired_activity": "Sports", "user_id": user_id}
        },
        {
            "action": "find_profile_swap",
            "explanation": "You can swap apartments with a resident near a Sports space. I found H5 near O12 (Sports) who likes your current activities.",
            "parameters": {"user_id": user_id, "swap_with": "H5", "target_space": "O12"}
        },
        {
            "action": "process_booking",
            "explanation": "You can book O10 for Sports if the slot is available and other residents agree.",
            "parameters": {"user_id": user_id, "space_id": "O10", "activity": "Sports"}
        }
    ]
    return {"context": context, "suggestions": suggestions}

#----------------------------------old gh_server_mediator.py flask call for negotiation
