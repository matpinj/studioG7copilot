...
class ChatTab(QWidget):
    def __init__(self, endpoint, extra_fields=None):
        ...
        if "general_question" in self.endpoint:
            ...
            # "Show Geometry" button (to the right of dropdowns)
            self.show_geom_btn = QPushButton("Show Geometry")
            self.show_geom_btn.clicked.connect(self.toggle_geometry)
    ...
    def toggle_geometry(self):
        if self.show_geom_btn.text() == "Show Geometry":
            ...
            try:
                r = requests.post("http://localhost:5000/hide_geometry_by_key")
                if r.status_code == 200:
                    self.chat_display.append("<b>Geometry Hidden</b>")
                else:
                    self.chat_display.append("<b>Error hiding geometry</b>")
            except Exception as e:
                self.chat_display.append(f"<b>Error:</b> {e}")
...
# Add additional tabs for geometry and visualization
        tabs.addTab(ChatTab("http://localhost:5002/geometry_suggestion"), "Modify Space")
        tabs.addTab(ChatTab("http://localhost:5003/visualize_space"), "Visualize Space")
