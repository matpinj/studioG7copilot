...
def send_udp_command_1(message, port=7000):
    """
    Improved UDP command sending with better error handling and timeout prevention
    This replaces the original simple version at the top of your file
    """
    try:
        if message is None:
            message = ""  # Ensure message is not None
            
        print(f"Sending to port {port}: '{message}'")  # Debug print
        
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.settimeout(1.0)  # Set timeout to prevent hanging - THIS IS KEY!
        
        message_bytes = str(message).encode("utf-8")
        sock.sendto(message_bytes, ("127.0.0.1", port))
        sock.close()
        
        print(f"Successfully sent UDP message: '{message}'")
        return True  # Success indicator
        
    except socket.timeout:
        print(f"UDP timeout sending to port {port} - receiver might not be listening")
        return False
...
# REMOVE the old version that looks like this:
# def send_udp_command_1(message, port=7000):
#     print(f"Sending to port {port}: {message}")  # Debug print
#     sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
# OPTIONAL: Even better version with async sending to prevent UI blocking
import threading
import queue
import time

class AsyncUDPSender:
    """Non-blocking UDP sender to prevent UI freezing"""
    def __init__(self):
        self.message_queue = queue.Queue(maxsize=10)  # Limit queue size
    ...
    def _worker(self):
        """Background worker thread for sending UDP messages"""
        while True:
            try:
                message, port = self.message_queue.get(timeout=1)
                
                # Skip duplicate messages sent too quickly
                current_time = time.time()
                if message == self.last_message and (current_time - self.last_time) < 0.1:
                    continue
                ...
                sock.sendto(str(message).encode("utf-8"), ("127.0.0.1", port))
                sock.close()
                print(f"Async UDP sent: '{message}' to port {port}")
                
            except queue.Empty:
                continue  # No messages, keep waiting
            except Exception as e:
                print(f"Async UDP error: {e}")
    
    def send(self, message, port=7000):
        """Send message asynchronously (non-blocking)"""
        try:
            self.message_queue.put_nowait((message, port))
            return True
        except queue.Full:
            print("UDP queue full, message dropped")
            return False

# Create global async sender
_async_udp_sender = AsyncUDPSender()

def send_udp_command_1_async(message, port=7000):
    """
    Async version that won't block your UI
    Use this if you're still experiencing slowness
    """
    return _async_udp_sender.send(message, port)
...
class SpaceQnAUI(QMainWindow):
    ...
    def handle_swap_response(self, data, house_key):
        ...
        self.neg_display.append(html)
         ...
         # --- NEW: Update port 7000 with detected keys from swap result ---
        keys_str = "|".join(sorted(extract_space_keys(result)))
        send_udp_command_1(keys_str, port=7000)
        self.neg_display.append(f"<div class='bubble system-bubble'><b>Detected keys from swap:</b> {keys_str or 'None'}</div>")
